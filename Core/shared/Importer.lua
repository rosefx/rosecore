...
function import(...)
    ...
    if (cArgs[1] == true) then
        ...
        return [[
        local cImports, isCompleteFetch = exports['rosecore']:import(true, ]]..cArgs..[[);
        if not cImports then return false end
        local cReturns = (not isCompleteFetch and {}) or false
        for i = 1, #cImports, 1 do
            local j = cImports[i]
            assert(load(j.rw))()
            if cReturns then cReturns[(#cReturns + 1)] = rose[(j.index)] end
        end
        if isCompleteFetch then return rose
        else return cReturns end
        ]]
    end
    return false;
end
